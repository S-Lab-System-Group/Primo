...
class DQNPolicy:
    def __init__(self, env, model_path, parameters, n_batch_rollouts):
        ...
        self.actor = ActorNetwork(
            self.sess,
            state_dim=[parameters["S_INFO"], parameters["S_LEN"]],
            action_dim=parameters["A_DIM"],
            learning_rate=parameters["ACTOR_LR_RATE"],
        )
        self.critic = CriticNetwork(
            self.sess, state_dim=[parameters["S_INFO"], parameters["S_LEN"]], learning_rate=parameters["CRITIC_LR_RATE"]
        )

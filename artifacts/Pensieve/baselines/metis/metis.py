...
def learn_dt(leaf_nodes):
    ...
    is_reweight = False
    n_test_rollouts = 50
    save_dirname = "./metis_model"
    save_fname = "metis_" + leaf_nodes + ".pk"
    save_viz_fname = "metis_" + leaf_nodes + ".svg"
    is_train = True
    ...
    parameters["VIDEO_SIZE_FILE"] = "../video_size_"
    parameters[
        "S_INFO"
    ] = 6  # bit_rate, buffer_size, next_chunk_size, bandwidth_measurement(throughput and time), chunk_til_video_end
    ...
    # Data structures
    env = Environment(all_cooked_time=all_cooked_time, all_cooked_bw=all_cooked_bw, parameters=parameters)
    ...
    end_time = time.time()
    print(f"Time Usage = {end_time - start_time} s")
